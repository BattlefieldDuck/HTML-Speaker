...
[].concat($jscomp.arrayFromIterable(speakers)).forEach(function(a){var b=a.hasAttribute("data-start")?a.getAttribute("data-start"):"\u25b6",d=a.hasAttribute("data-pause")?a.getAttribute("data-pause"):"\u25cf";a.innerHTML=b+a.innerHTML;a.addEventListener("click",function(){if(a!==activeSpeaker){a.innerHTML=a.innerHTML.replace(b,d);null!==activeSpeaker&&(activeSpeaker.innerHTML=activeSpeaker.innerHTML.replace(activeSpeaker.hasAttribute("data-pause")?activeSpeaker.getAttribute("data-pause"):"\u25cf",
...
synth.getVoices(),g=a.getAttribute("data-voice");if(null!==f){f=$jscomp.makeIterator(f);for(var e=f.next();!e.done;e=f.next())if(e=e.value,e.name===g){c.voice=e;break}}null===c.voice&&console.warn("HTML-Speaker: voice("+g+") is not found in the browser. Default voice will be used.")}c.onend=function(){a.innerHTML=a.innerHTML.replace(a.hasAttribute("data-pause")?a.getAttribute("data-pause"):"\u25cf",a.hasAttribute("data-start")?a.getAttribute("data-start"):"\u25b6");a===activeSpeaker&&(activeSpeaker=
null)};synth.speak(c);activeSpeaker=a}else synth.paused?(a.innerHTML=a.innerHTML.replace(b,d),synth.resume()):synth.speaking&&(a.innerHTML=a.innerHTML.replace(d,b),synth.pause())})});